function setZoom(type, value) {
  console.log("base", zoomContentXY);
  ...
  switch (type) {
    case "zoomIn":
      if (value) {
        scale = Math.min(3.5, scale + value);
      } else {
        scale = Math.min(3.5, scale + 0.5);
      }
  ...
  console.log("scale", zoomContent.x, zoomContent.y);
  if (scale === 1) {
...
function areaDragDown(e) {
  ...
  if (zoomContent.scaleX === 1 || zoomContent.scaleX === -1) return;
